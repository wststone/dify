...
let prevConfig: null | CONFIG = localStorage.getItem(CONFIG_KEY)
  ? (JSON.parse(localStorage.getItem(CONFIG_KEY) as string) as CONFIG)
  : null;
...
const Main: FC<IMainProps> = () => {
  ...
  const getInitConfig = (type: "model" | "plugin") => {
    if (type === "model") {
      return {
        providerName:
          prevConfig?.providerName ||
          agentThoughtModelList?.[0]?.model_provider.provider_name,
        modelId: prevConfig?.modelId || agentThoughtModelList?.[0]?.model_name,
      };
    }

    if (type === "plugin") return prevConfig?.plugin || DEFAULT_PLUGIN;
  };

  useEffect(() => {
    document.title = `${t("explore.sidebar.chat")} -  乐学一百`;
  }, []);
  /*
   * app info
   */
  ...
  const [speechToTextConfig, setSpeechToTextConfig] =
    useState<SuggestedQuestionsAfterAnswerConfig | null>(null);
  const [citationConfig, setCitationConfig] =
    useState<SuggestedQuestionsAfterAnswerConfig | null>(null);
  ...
  const conversationName =
    currConversationInfo?.name ||
    (t("share.chat.newChatDefaultName") as string);
    ...
    if (!isNewConversation) {
      ...
      if (modelConfig) {
        ...
        modelConfig.agent_mode.tools.forEach((item: any) => {
          ...
          if (pluginName === "dataset") datasetIds.push(item.dataset.id);
          else pluginConfig[pluginName] = item[pluginName].enabled;
        });
  ...
  const handleSend = async (message: string) => {
    if (isNewConversation) {
      const isModelSelected =
        modelId &&
        !!agentThoughtModelList.find((item) => item.model_name === modelId);
      if (!isModelSelected) {
        notify({
          type: "error",
          message: t("appDebug.errorMessage.notSelectModel"),
        });
        return;
      }
      setPrevConfig({
    ...
    sendChatMessage(data, {
      ...
      onThought(thought) {
        ...
        (responseItem as any).agent_thoughts = [
          ...(responseItem as any).agent_thoughts,
          thought,
        ]; // .push(thought)
        // has switched to other conversation
  ...
  useEffect(() => {
    (async () => {
      if (controlChatUpdateAllConversation && !isNewConversation) {
        ...
        const item = allConversations.find(
          (item) => item.id === currConversationId
        );
        setAllConversationList(allConversations);
        if (item) {
  ...
  const renderSidebar = () => {
    if (!APP_ID || !promptConfig) return null;
    return (
      <Sidebar
  ...
  const [modelId, setModeId] = useState<string>(
    (initConfig as any)?.modelId as string
  );
  const [providerName, setProviderName] = useState<ProviderEnum>(
    (initConfig as any)?.providerName as ProviderEnum
  );
  // const currModel = MODEL_LIST.find(item => item.id === modelId)

  const [plugins, setPlugins] = useState<Record<string, boolean>>(
    getInitConfig("plugin") as Record<string, boolean>
  );
  ...
  const isCurrConversationPinned = !!pinnedConversationList.find(
    (item) => item.id === currConversationId
  );
                    ...
                    <ItemOperation
                      ...
                      togglePin={() =>
                        isCurrConversationPinned
                          ? handleUnpin(currConversationId)
                          : handlePin(currConversationId)
                      }
                      isShowDelete
