...
export type IChatProps = {
  ...
  controlFocus?: number;
  showOpeningSuggestions?: boolean;
...
const Chat: FC<IChatProps> = ({
  ...
  const sendBtn = (
    <div
      className={cn(
        !(!query || query.trim() === "") && s.sendBtnActive,
  ...
  return (
    <div className={cn("px-3.5", "h-full")}>
      ...
      <div
        className={cn(
          isShowConfigElem && configElem ? "h-0" : "h-full",
          "space-y-[30px]"
        )}
      >
        {openingSuggestions && openingSuggestions.length > 0 && (
          <div className="flex flex-wrap gap-2 overflow-x-auto pb-2">
            {openingSuggestions.map((item, index) => (
              <div key={item} className="shrink-0 flex justify-center">
        ...
        {chatList.map((item) => {
          if (item.isAnswer) {
            ...
            const thoughts = item.agent_thoughts?.filter(
              (item) => item.thought !== "[DONE]"
            );
            const citation = item.citation;
            const isThinking =
              !item.content &&
              item.agent_thoughts &&
              item.agent_thoughts?.length > 0 &&
              !item.agent_thoughts.some((item) => item.thought === "[DONE]");
            return (
              <Answer
                key={item.id}
                item={item}
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="absolute top-0 right-2 flex items-center h-[48px]">
              ...
              {query ? (
                <div
                  className="flex justify-center items-center w-8 h-8 cursor-pointer hover:bg-gray-100 rounded-lg"
                  onClick={() => setQuery("")}
                >
