...
const Configuration: FC = () => {
  ...
  const syncToPublishedConfig = (_publishedConfig: PublichConfig) => {
    ...
    setOpeningSuggestions(modelConfig.opening_suggestions || []);
  ...
  useEffect(() => {
    if (hasFetchedDetail && !modelModeType) {
      const mode = textGenerationModelList.find(
        ({ model_name }) => model_name === modelConfig.model_id
      )?.model_mode;
      if (mode) {
  ...
  useEffect(() => {
    fetchAppDetail({ url: "/apps", id: appId }).then(async (res) => {
      ...
      if (promptMode === PromptMode.advanced) {
        setChatPromptConfig(
          modelConfig.chat_prompt_config ||
            (clone(DEFAULT_CHAT_PROMPT_CONFIG) as any)
        );
        setCompletionPromptConfig(
      ...
      if (modelConfig.opening_suggestions) {
        setOpeningSuggestions(modelConfig.opening_suggestions);
      }
      ...
      const config = {
        modelConfig: {
          ...
          opening_suggestions: modelConfig.opening_suggestions || [],
          ...
          <div className="flex items-center justify-between px-6 shrink-0 h-14">
            ...
            <div className="flex items-center">
              ...
              <Button
                ...
                className="shrink-0 mr-2 w-[70px] !h-8 !text-[13px] font-medium"
              >
                {t("appDebug.operation.resetConfig")}
              </Button>
              <Button
                type="primary"
                onClick={() => handlePublish(false)}
                className={cn(
                  cannotPublish && "!bg-primary-200 !cursor-not-allowed",
